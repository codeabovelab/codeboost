...
| For a declared pre-existing symlink, resolve its target identity without following filesystem links and reject the item if that target is also a writable declared entry, or is a directory containing any writable declared descendant, in the same invocation. Use filesystem identity keys and path-component ancestry (not string-prefix comparison); for example `link -> target/` conflicts with `target/file`. A link and its target cannot both be edited under one item; split legitimate changes into separate dependent items. Managed link operations use unlink/recreate or rename without dereferencing the link. Before launch, record a no-follow snapshot of every declared link target (including all descendants for directory targets and absent-path identity for dangling links), for both its old and proposed target. If a proposed target is only known after the run, compare against the trusted pre-run tree and filesystem snapshot. Before any scope classification, test, or commit, reject every observed pre/post change to these target entries: content, mode, type, creation, deletion, or descendant changes, whether declared or not. Such changes are safety violations and must never enter the safe out-of-scope commit path. Test writes through a link to an undeclared file, directory child, and dangling target; all must stop before checks/commit. Post-run path diffs alone cannot prove which spelling an arbitrary program used to write. | Failure |
...
**Accepted state versus runtime containment.** The symlink target rules and snapshots validate the pre-run and accepted post-run repository states; they do not prove that an arbitrary program never created, used, and restored a transient link. A proposed target discovered after execution is never retroactive authorization for access. During execution, the container mount/permission and network boundaries govern access: no unrelated host paths are mounted, metadata/schema/auth mounts are read-only, and only bounded worktree/scratch are writable. A link cannot confer permissions beyond that namespace. The selected vendor credential is deliberately readable by its CLI inside that namespace, including by a compromised CLI; merely spelling that same mounted path through a symlink is not an additional privilege. We do not promise that arbitrary execution cannot read its own vendor credential or access writable scratch. Tests must attempt transient links to unmounted host paths and writes to protected mounts, restore the links, and still confirm the runtime boundary denied those accesses. Do not use a final snapshot as evidence that transient access was blocked.
