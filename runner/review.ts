...
export class ReviewService {
  ...
  load() {
    ...
    const segments = applyChoices(plan, raw, saved.choices, identity).map((segment, index) => {
      ...
      return { ...segment, key: keys[index]!, originalRow: raw[index]!.row };
    });
    ...
    const items = plan.items.map(item => {
      ...
      return { ...item, state: states[item.id], count: owned.length, reasons, before,
  ...
  act(input: unknown) {
    ...
    if (command.action === 'approve' && typeof command.item === 'string') {
      ...
      this.store.saveReview(identity, view.expected, [], [{ key: segment.key, action: command.action, item }]);
