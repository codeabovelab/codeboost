...
| Preparation still writing at restart (review round 32) | Crashed storage allocator's group still alive → restart → step 2a kills and awaits it → only then D recovery and export run |
| Renamed database (review round 32) | Runner A holds the lock → the database file is renamed → runner B starts through the new name → same inode, same lock file → B exits |
