...
0. **Before any lock file is opened, created or removed:** resolve the parent directory with `realpath`, and check that it is owned by the current user and not writable by group or others. Otherwise exit. Only then is the lock path a safe boundary, including for stale-lock removal.
