...
The GitHub adapter reads the current PR base/head, effective branch rulesets, classic branch protection, required check contexts and app identities, and issue cross-references. It ignores review records that are not required checks. An unreadable or incomplete rule source, a pending/missing/failing check, another open or merged PR for the issue, a conflict, or a closed PR blocks merging.

Automatic merging also requires a server-enforced current-base policy: a merge queue or strict required-status-check rule. A client-side fetch cannot supply that guarantee. The coordinator performs the complete gate twice, verifies the same base/head pair, and invokes `gh pr merge --match-head-commit` with literal argv. GitHub's refusal text is returned to the local UI. There is no “merge anyway” path around missing atomic protection.
