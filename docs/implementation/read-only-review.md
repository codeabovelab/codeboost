...
**Superseded 2026-09-26:** Ask no longer runs the host CLI. It runs in the lane D container in the read-only "questions" phase, with a clone of the reviewed head at `/work`; Claude needs `CLAUDE_CODE_OAUTH_TOKEN` and Codex its `auth.json` (see `agent-isolation.md`, "First consumer: Ask"). The rest of this paragraph describes the original host adapter.
